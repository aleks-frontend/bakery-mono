...
Order list pagination is server-side (`GET /api/orders` also takes `page` and `pageSize`, the latter restricted to `ORDER_PAGE_SIZES` — 10/25/50 — from `@bakery/schemas`; the response shape is `{ data: Order[], total: number }` rather than a bare array). `OrdersPage` resets `page` to 1 whenever any filter/sort/pageSize changes.

**Row selection spans pages, not just the current one** — this bit us once already (the header "select all" checkbox originally toggled TanStack Table's own row-selection state, which is scoped to whatever `data` the table was given; once pagination landed that silently became "select this page only," and the baker's workshop-list/package-stickers PDFs undercounted quantities without any error, since both are generated from `selectedOrders`). Fixed by lifting selection out of `OrdersTable` entirely: `OrdersPage` owns `rowSelection: RowSelectionState` (keyed by order id) and passes it down as a controlled prop, along with `allSelected`/`someSelected` computed against the query's `total` rather than TanStack's page-scoped `getIsAllRowsSelected()`. The header checkbox's `onSelectAllToggle` either clears the selection or fetches every order matching the current filters via `GET /api/orders?all=true` (bypasses `page`/`pageSize` server-side) and selects all of them — not just the visible page. A running `orderCache: Map<id, Order>` in `OrdersPage` resolves full `Order` data for any selected id regardless of which page (if any) it's currently rendered on, since bulk actions and PDF/XLS generation need full order data, not just ids. Selection is cleared on any filter/sort/pageSize change and after any bulk mutation succeeds (delete/archive/unarchive/status-change) so a stale selection can't silently be reused for a later action.
