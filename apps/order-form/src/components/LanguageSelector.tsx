...
export function LanguageSelector() {
  ...
  const getInitialLanguage = (): string => {
    ...
    return lang.split("-")[0] ?? "en"; // Handle locale codes like "en-US" -> "en"
  };
  ...
  useEffect(() => {
    const handleLanguageChanged = (lng: string) => {
      const langCode = lng.split("-")[0] ?? "en"; // Handle locale codes like "en-US" -> "en"
      ...
      {LANGUAGES.map(({ code, titleKey, flag }) => {
        ...
        return (
          <button
            ...
            className={`
            w-9 h-9 border-2 rounded-xl bg-bakery-card flex items-center justify-center
            transition-all duration-200 shadow-sm p-0
            hover:-translate-y-0.5 hover:shadow-md hover:border-bakery-primary
            md:w-9 md:h-9
            ${
              active
                ? "border-bakery-primary bg-bakery-primary shadow-[0_0_0_3px_rgba(196,112,59,0.2)]"
