...
## Phase 29 — Cycle-Start Reminder Emails via a Persistent Opt-Out Mailing List

Reminder only, not yet scoped in detail — client explicitly wants to discuss this further once work on it actually starts (targeted for next week), so nothing below is a final decision, just what's been captured so far across two rounds of notes.

**Original ask**: when the baker starts a new cycle, send a "reminder" email to customers who ordered recently, letting them know ordering is open again. Repeating-order customers are deliberately excluded — they already get their own confirmation email each cycle from `cloneRepeatingOrdersIntoCycle`'s existing `sendOrderNotifications` call, so a reminder on top would be a duplicate.

**Client follow-up (supersedes the "just query last cycle's orders" framing below)**: this isn't a per-cycle derived query, it's a persistent, cumulative mailing list with an opt-out:

- Whenever a customer places an order that is **not** marked repeating, add their email to a "reminders" list (if not already on it) — so the list accumulates across every cycle, not just the most recent one.
- Whenever a cycle starts, email everyone currently on the reminders list.
- Every reminder email includes a link with the text "I don't want to get these reminders anymore" — clicking it moves that email to a separate list of people who've opted out, and they must never be emailed again.
- Ongoing reconciliation: if an email on the reminders list later becomes tied to a repeating order, it should be removed from the reminders list at that point (or filtered out at send time) — that customer is now getting notified every cycle anyway via the repeating-order clone email, so a reminder would be redundant. Client's framing was "always compare the list of emails with repeating emails and remove it if it is a repeating order," i.e. this needs to be checked continuously/at each send, not just once.

**New open design questions from the follow-up (in addition to the ones below):**

- [ ] Data model — one table with an `unsubscribedAt`-style nullable timestamp, or two genuinely separate lists (subscribed / opted-out) as the client's wording suggests? Either satisfies the behavior; not decided yet.
- [ ] Scope of "when user makes an order" — only the public order-form's `POST /api/public/orders`, or also admin-created manual orders? The ask says "when user makes an order," not "when a customer submits the public form," so this needs confirming.
- [ ] Unsubscribe link mechanics — can't be a plain `?email=...` link (anyone who knows or guesses an email could unsubscribe someone else); needs an opaque per-recipient token and a new unauthenticated public endpoint to handle the click
- [ ] Repeating-order reconciliation timing — remove from reminders permanently the first time an email is ever tied to a repeating order, or just suppress the reminder for as long as an active `RepeatingOrder` exists for that email (rejoining the reminders list automatically if that repeating order is later deleted)?
- [ ] Is opting out permanent, or does placing a brand-new order after unsubscribing put someone back on the reminders list? Not addressed yet either way.
- [ ] Backfill question: does the reminders list start empty from whenever this ships (only accumulating going forward), or get seeded once from existing historical order data? Affects the very first send's size.
- [ ] Whether the admin panel needs any visibility into these lists (view subscriber/opt-out counts, manually remove someone) — not asked for yet, but likely to come up once this is live
- [ ] Copy/design for the reminder email itself (subject, tone, whether to link directly to the order form, how the unsubscribe line is worded/placed) — not drafted yet
- [ ] Decide whether the send blocks the `POST /api/cycles` response (like `cloneRepeatingOrdersIntoCycle` does today) or fires fully fire-and-forget — given the volumes below, fire-and-forget (matching `sendOrderNotifications`'s existing pattern) seems right so a slow email batch never delays the baker's "cycle started" confirmation

**Earlier design questions, now partly superseded by the above (kept for reference, not re-derived):**

- [ ] ~~Define "previous cycle" precisely~~ — less relevant now that the list is cumulative rather than derived fresh from "last cycle's orders" each time, though the backfill question above raises a similar issue for the very first send
- [ ] `Order.repeatingOrderId IS NULL` still correctly identifies a non-repeating order at the moment it's placed (excludes both repeating-order clones and the original order that first enrolled as repeating), so this remains the right check for "should this order's email be added to the reminders list"
- [ ] **`Order` has no persisted `locale`** — still an open gap. `locale` only exists transiently on the create-order request payload (used once to pick email copy at send time), never stored on the `Order` row itself, so a reminder sent later has no record of which language a customer's confirmation used. Options: default every reminder to one locale (mirroring `cloneRepeatingOrders.ts`'s existing default-to-`"sr"` workaround for the same underlying gap), or add a persisted `Order.locale` column now so every feature needing it going forward has it — worth deciding once, since this keeps resurfacing

**Resend impact — checked against real usage, since this runs on the free plan today (client explicitly asked this be discussed as part of this task):**

Free plan is 3,000 emails/month **and capped at 100/day**, whichever hits first ([Resend pricing](https://resend.com/docs/knowledge-base/what-is-resend-pricing)); paid tiers remove the daily cap entirely (Pro: $20/mo for 50,000 emails, ~$0.0008/email overage — [source](https://automationatlas.io/answers/resend-pricing-explained-2026/)).

Checked against this dev DB's real order history (recent cycles, `2026-W23`–`2026-W33`): ~70–100 orders/cycle, ~26–47 unique customer emails/cycle, repeating-order clones currently near-zero in this data (that feature is newer than most of this history). At today's volume:

- **Monthly total is not a concern**: existing per-order emails (customer confirmation when an email is given, plus one admin alert per order, always) run roughly 400–450/month. A weekly reminder batch adds more here than originally estimated, since the list is now cumulative rather than just "last cycle's ~30–47 unique emails" — it will keep growing (net of opt-outs and repeating-order removals) as more distinct customers place at least one order over time, so this number should be re-checked against actual accumulated-list size once the list model is built, not assumed to stay near 30–47 forever. Even so, likely still well under 3,000/month for the foreseeable future given current order volume.
- **The daily cap is the real risk, and it's concentrated on one specific day**: cycles open on Saturday (`project-scope.md`), which is exactly when "Start Next Cycle" gets clicked and this reminder batch would fire — the same day new orders for the *new* cycle also start arriving and generating their own confirmation/admin emails. Real data shows Saturday order volume swinging widely by week (9, 10, and 51 orders across three sampled Saturdays). A busy Saturday's own order emails (up to ~51 orders × up to 2 emails each) landing on the same day as the reminder burst could realistically approach or exceed the 100/day cap in a high-volume week — more so once the reminders list has grown past its initial ~30–47-email size.
- [x] **Client has already flagged this as something we might need to split across two days** ("we will still check the pricing and might break into two days") — i.e. leaning toward the spread-the-send mitigation already noted below, to be confirmed once real list size is known.
- [ ] Mitigation to decide on before shipping: spread the reminder send over the following day or two instead of firing it all at once at cycle-start (a reminder has no real-time urgency the way an order confirmation does), keep monitoring actual Saturday volume via the Resend dashboard and only act if a week gets close to the cap, or move to Resend's Pro plan (removes the daily cap outright for $20/mo) if this becomes a recurring concern
